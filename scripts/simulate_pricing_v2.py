...
def map_finishes_to_modifier(finishes):
    ...
    for f in finishes:
        ...
        # ITM (In The Money but not podium) = 0
    return mod
...
def simulate_price(player, course):
    ...
    return {
        ...
        "new": new_price,
        "rating_mod": f"{rating_modifier}%",
        "form_mod": f"{form_modifier}%",
        "total_pct": f"{total_percentage_change}%",
        "finishes": ",".join(player['recent_finishes'])
    }

print("=== BUDGET TRIVIA: $950 Cap / 6 Roster Spots = ~$158 Average Budget Per Player ===")
print("\n=== NEW LONDON TECH (Tight Woods, Tech=5, Dist=3) ===")
print(f"{'Player':<16} | {'Base':<5} | {'New':<5} | {'RateMod':<8} | {'FormMod':<8} | {'Total%':<6} | {'Recent Form'}")
print("-" * 90)
for p in players:
    res = simulate_price(p, new_london)
    print(f"{res['name']:<16} | ${res['base']:<4} | ${res['new']:<4} | {res['rating_mod']:<8} | {res['form_mod']:<8} | {res['total_pct']:<6} | {res['finishes']}")
